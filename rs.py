...
# Ensure Streamlit Secrets exist before proceeding
if "gee_private_key" not in st.secrets:
    st.error("Google Earth Engine credentials not found in Streamlit secrets!")
    st.stop()

# Reconstruct credentials dictionary
...
# Authenticate with GEE using ee.Credentials
try:
    credentials = ee.Credentials.from_service_account_info(service_account_info)
